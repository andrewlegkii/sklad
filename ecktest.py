...
TABLE_FILE = r"C:\Python\sklad\sklad\Екатеринбург - учет оборота поддонов.xlsx"
...
# === ПОЛУЧАТЕЛЬ И ОТПРАВИТЕЛЬ НАПОМИНАНИЙ ===
REMINDER_RECIPIENT = "skoppss@yandex.ru"
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            if ("Х5" in supplier_lower or "х5" in supplier_lower or "дистр" in supplier_lower) and today_msk == parsed_date:
                ...
                if time(16, 16) <= current_time_msk < time(16, 18):
                    logging.info("⏰ Время 12:00 — проверяю, нужно ли отправлять напоминание")
                    if not (has_driver and has_tractor):
                        ...
                        if key not in sent_reminders:
                            ...
                            send_email(subject, body, REMINDER_RECIPIENT)
                            sent_reminders.add(key)
                            logging.info(f"✅ ✉️ ОТПРАВЛЕНО напоминание для {net_name} на {REMINDER_RECIPIENT} от {SENDER_EMAIL}")
                        else:
                    ...
                    logging.info(f"⏳ Время {current_time_msk} — не 12:00. Пропуск.")
            ...
            # === Тандер: напоминание в 14:00 по МСК накануне ===
                ...
                if today_msk == reminder_date:
                    ...
                    if time(14, 0) <= current_time_msk < time(14, 1):
                        logging.info("⏰ Сейчас 14:00–14:01 по МСК — проверяю необходимость отправки напоминания")
                        if not (has_driver and has_tractor):
                            ...
                            if key not in sent_reminders:
                                ...
                                send_email(subject, body, REMINDER_RECIPIENT)
                                sent_reminders.add(key)
                                logging.info(f"✅ ✉️ ОТПРАВЛЕНО напоминание для ТАНДЕР на {REMINDER_RECIPIENT} от {SENDER_EMAIL}")
                            else:
                        ...
                        logging.info(f"⏳ Время {current_time_msk} — не в окне 14:00–14:01. Пропуск.")
                else:
